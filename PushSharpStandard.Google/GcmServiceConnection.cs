...
namespace PushSharpStandard.Google
{
    ...
    public class GcmServiceConnection : IServiceConnection<GcmNotification>
    {
        ...
        async Task processResponseOk(HttpResponseMessage httpResponse, GcmNotification notification)
        {
            ...
            var json = JsonSerializer.Deserialize<GcmResponse>(str);

            result.NumberOfCanonicalIds = json.NumberOfCanonicalIds;
            result.NumberOfFailures = json.NumberOfFailures;
            result.NumberOfSuccesses = json.NumberOfSuccesses;
            ...
            foreach (var r in jsonResults)
            {
                ...
                msgResult.MessageId = r.MessageId;
                msgResult.CanonicalRegistrationId = r.CanonicalRegistrationId;
                ...
                {
                    msgResult.ResponseStatus = GetGcmResponseStatus(r.Error);
                }
            ...
            foreach (var r in result.Results)
            {
                ...
                if (r.ResponseStatus == GcmResponseStatus.Ok)
                { // Success
                    multicastResult.Succeeded.Add(singleResultNotification);
                }
                else if (r.ResponseStatus == GcmResponseStatus.CanonicalRegistrationId)
                ...
                else if (r.ResponseStatus == GcmResponseStatus.Unavailable)
                { // Unavailable
                ...
                else if (r.ResponseStatus == GcmResponseStatus.NotRegistered)
                { //Bad registration Id
                    var oldRegistrationId = string.Empty;
